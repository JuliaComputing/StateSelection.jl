...
Among the equations in `active_eqs`, find an equation `ieq` which is solvable for
exactly one variable `ivar` in `active_vars`. The variable must also satisfy `condition`.
If such an equation is found, then `var_eq_matching[ivar]` is updated to `ieq`. Returns
`ieq`, or `0` if no such equation is found. `solvable_graph` is a bipartite graph
from equations to variables, denoting which equations are solvable for which variables.
In the context of the paper, `solvable_graph` is the subgraph composed of **bold** edges
of the associated bipartite graph of the DAE.
"""
function find_single_solvable_eq!(
        solvable_graph::BipartiteGraph{Int, Nothing}, var_eq_matching::MatchingT,
        active_vars::Set{Int}, active_eqs::Set{Int}, condition::F = _ -> true
    ) where {F}
    for ieq in active_eqs
        first_solvable_var = 0
        has_multiple_solvable = false
        for ivar in 𝑠neighbors(solvable_graph, ieq)
            ivar in active_vars || continue
            condition(ivar) || continue

            if iszero(first_solvable_var)
                first_solvable_var = ivar
            else
                has_multiple_solvable = true
                break
            end
        end
        has_multiple_solvable && continue
        iszero(first_solvable_var) && continue
    ...
    while !isempty(active_vars)
        ...
        if alg.isder !== nothing
            single_solvable_eq = find_single_solvable_eq!(
                solvable_graph, var_eq_matching, active_vars, active_eqs, alg.isder
            )
        ...
        single_solvable_eq = find_single_solvable_eq!(
            solvable_graph, var_eq_matching, active_vars, active_eqs
        )
        if !iszero(single_solvable_eq)
            delete!(active_eqs, single_solvable_eq)
            delete!(active_vars, invview(var_eq_matching)[single_solvable_eq])
